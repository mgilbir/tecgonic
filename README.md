...
Compilation is CPU-bound. Three levers trade correctness headroom for speed —
the context-cancellation cost, pass capping (`WithMaxPasses`), and state seeding
(`WithStateDir`) — and the engine runs on our optimized [wazero](https://wazero.io/)
fork. All are covered in **[docs/performance.md](docs/performance.md)**.
